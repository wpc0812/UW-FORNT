  ...
  data() {
    ...
    var carcountAllEcc = (rules, value, callback) => {
      if (!value) {
        ...
        callback(new Error("车辆总数大于等于异地车数"));
    ...
    var foreigncarcountEcc = (rules, value, callback) => {
      if (!value) {
        ...
        callback(new Error("异地车数是0到999999之间的整数"));
      } else if (value && !/^\+?[1-9][0-9]*$/.test(value)) {
        callback(new Error("只能输入正整数"));
      } else if (parseInt(value) < 0) {
        ...
        callback(new Error("车辆总数应小于异地车辆数"));
      } else if (
        parseInt(value) <= parseInt(this.UwMotorcadeMainVO.carcountAll) &&
    ...
    return {
      ...
      rules: {
        ...
        insuredflag: [
          { required: true, message: "控制关系人标志必选", trigger: ["blur"] }
        ],
        insuredName: [
